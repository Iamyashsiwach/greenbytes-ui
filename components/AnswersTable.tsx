      ...
      <CardContent className="p-4">
        <div className="space-y-3 max-h-64 overflow-y-auto custom-scrollbar">
          {questions.map((question, index) => {
            ...
            return (
              <div key={question.key} className="flex items-start justify-between gap-3 p-2 rounded-lg bg-muted/50">
                ...
                <Badge variant={answerInfo.variant} className="flex items-center gap-1 shrink-0">
                  {answerInfo.icon}
                  {answerInfo.label}
                </Badge>
              </div>
